...
class UploadHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/":
            ...
            self.send_header('Content-Type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({"message": "Welcome to 'Image Hosting Server'"}).encode())
            return
        ...
        if self.path == "/upload":
            ...
            try:
                ...
                self.send_json_error(404, "Images directory not found.")
                return
            except PermissionError:
                self.send_json_error(403, "Permission denied to access images directory.")
                return
