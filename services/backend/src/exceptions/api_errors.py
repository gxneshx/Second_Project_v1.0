"""The custom API exception classes for upload server.

It defines structured error types for specific failure scenarios such as
...
class NotSupportedFormatError(APIError):
    ...
    def __init__(self, supported_formats: set[str]):
        ...
        message = f'Unsupported file format. Supported ones: {formats_list}.'
...
class MultiFilesUploadError(APIError):
    ...
    def __init__(self):
        message = "Only one file can be uploaded in a single request."
        super().__init__(message)